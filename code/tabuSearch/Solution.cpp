...
void Solution::localSearch(int maxSteps, double LS_limit, double prob1, double prob2, double prob3)
{
  ...
  for( int i = 0; evCount < data->n_of_events; i = (i+1)% data->n_of_events){
	...
	if( neighbourAffectedHcv < currentAffectedHcv){
	  //std::cout<<"current hcv " << computeHcv() << "neighbour " << neighbourSolution->computeHcv()<< std::endl;
  ...
  if(feasible){ // if the timetable is feasible
	...
	break;
      ...
      //std::cout << "event " << eventList[i] << " cost " << currentScv<<std::cout;
	...
	if(rg->next() < prob1){ // each with given propability
	  ...
	  if(neighbourHcv == 0){ // consider the move only if no hcv are introduced
	    //std::cout<< "reintroduced hcv" << neighbourSolution->computeHcv()<< std::endl;
	    ...
	    if( neighbourScv < currentScv){
	      //std::cout<<"current scv " << computeScv() << "neighbour " << neighbourSolution->computeScv()<< std::endl;
	      copy( neighbourSolution );
	      delete neighbourSolution;
	      evCount = 0;
	      foundbetter = true;
...
void Solution::assignRooms(int t)
{
  val.clear();
  dad.clear();
  std::vector<int> assigned; // vector keeping track for each event if it is assigned or not
 ...
 // 	computeScv();
	...
	while(timer.elapsedTime(Timer::VIRTUAL) < timeLimit)
	{
	  ...
	  if (currentTime - printTime > TIMEINTERVALPRINT) {
	    ...
	    std::cout << "time left: " << timeLimit - currentTime << std::endl;
	  }
...
//					    std::cout<< "Hcv" << newHcv << std::endl;
		...
		//std::cout<< "Hcv" << hcv << " Scv" << scv << std::endl;
...
//delta1Compute() computes the hcv of the neighborSolution obtained by applying Move1
//if the neighborSolution is feasible then it computes the scv;
//this function modifies neighborSolution->hcv and neighborSolution->scv
void Solution::delta1Compute(Solution * neighbourSolution, int i, int t, int t_orig)
{
	if(hcv == 0) //if the current solution is feasible
	{
		neighbourSolution->hcv = neighbourSolution->eventAffectedHcv(eventList[i]);
	}else
	{
		int neighbourAffectedHcv = neighbourSolution->eventAffectedHcv(eventList[i]) +
		...
		int currentAffectedHcv = eventAffectedHcv(eventList[i]) + affectedRoomInTimeslotHcv(t);
		int delta1Hcv = neighbourAffectedHcv - currentAffectedHcv;
    	neighbourSolution->hcv = hcv + delta1Hcv;
	}

	if(neighbourSolution->hcv==0)
	{ // only if no hcv are introduced by the move
 		int neighbourScv = neighbourSolution->eventScv(eventList[i]) +   // respectively Scv involving event e
						   singleClassesScv(eventList[i]) -  // + single classes introduced in day of original timeslot
						   neighbourSolution->singleClassesScv(eventList[i]); // - single classes "solved" in new day
		int delta1Scv = neighbourScv - eventScv(eventList[i]);
		neighbourSolution->scv = scv + delta1Scv;
	}
...
//delta2Compute() computes the hcv of the neighborSolution obtained by applying Move2
//if the neighborSolution is feasible then it computes also the scv;
//this function modifies neighborSolution->hcv and neighborSolution->scv
void Solution::delta2Compute(Solution * neighbourSolution, int i, int j)
{
	if(hcv == 0) //if feasible
	{
		neighbourSolution->hcv = neighbourSolution->eventAffectedHcv(eventList[i]) +
	...
	}else
	{
		...
		int currentAffectedHcv = eventAffectedHcv(eventList[i]) + eventAffectedHcv(eventList[j]);
		int delta2Hcv = neighbourAffectedHcv - currentAffectedHcv;
		neighbourSolution->hcv = hcv + delta2Hcv;
	}
	...
	{ // only if no hcv are introduced by the move
		  ...
		  int delta2Scv = neighbourScv - ( eventScv(eventList[i]) + eventScv(eventList[j]) );
		  neighbourSolution->scv = scv + delta2Scv;
	}
}

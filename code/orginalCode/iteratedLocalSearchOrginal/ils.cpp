...
int main( int argc, char** argv) {
  ...
  while( control.triesLeft()){
    ...
    while(control.timeLeft() && bestSolution->scv > 0){
      iterations++;
      if (control.getTime() - printTime > 20) { // If over 20 seconds has gone since last time-print, print again
        printTime = control.getTime();
	cout << "time left: " << control.timeLimit - control.getTime()<< endl;
	iterations = 0;
      }

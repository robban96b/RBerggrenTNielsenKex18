...
// stored in a std::vector of pairs of integers, one for each event,
// representing the timeslot and room assigned to that event respectively

class Solution{
  public:

  std::vector<std::pair<int, int> > sln; // std::vector of (timeslot, room) assigned for each event
  std::map<int, std::vector<int> > timeslot_events; // for each timeslot a std::vector of events taking place in it

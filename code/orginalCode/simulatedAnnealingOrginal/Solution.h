...
class Solution{
  ...
  std::vector<std::pair<int, int> > sln; // vector of (timeslot, room) assigned for each event
  std::map<int, std::vector<int> > timeslot_events; // for each timeslot a vector of events taking place in it
  ...
  void simulatedAnnealing_LS(std::ostream &outfile,double time_limit, double alfa, int nrep,
  ...
  void simulatedAnnealing_LS_Tfixed(std::ostream &outfile, double time_limit,
					    double TempFactorHcv, double TempFactorScv,
				    double prob1 = 1.0, double prob2 = 1.0, double prob3 = 0.0);
  void simulatedAnnealing_Random(std::ostream &outfile, double time_limit, double alfa, int nrep,
				 double iniTempFactorHcv, double iniTempFactorScv,
				 double rejectedLimHcv, double rejectedLimScv,
				 double prob1 = 1.0, double prob2 = 1.0, double prob3 = 0.0);
...
private:
  ...
  bool SAcriteria(int delta, double Temp);
};

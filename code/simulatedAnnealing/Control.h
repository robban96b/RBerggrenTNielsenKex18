...
private:
	...
	std::ostream *os;
	std::istream *is;
	char * readline (std::istream &infile);
...
public:
  ...
  void informationsReport(int method, double rateNeighbourhood, double alfa, double iniTempFactorHcv, double iniTempFactorScv, double rejectedLimHcv, double rejectedLimScv, double initialization);
  void setSAParameters(const int neighbourhoodSize, int &method, double &rateNeighbourhood, double &alfa, int &nrep, double &iniTempFactorHcv, double &iniTempFactorScv, double &rejectedLimHcv, double &rejectedLimScv);
};
